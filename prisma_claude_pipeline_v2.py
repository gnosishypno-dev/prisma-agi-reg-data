#!/usr/bin/env python3
"""
PRISMA Claude Pipeline V2
========================
Data collection pipeline for the PRISMA AI QRNG — Claude replication experiment.
Tests the Observer-Weight Hypothesis (PRISMA Framework)
using the ANU Quantum Random Number Generator and Claude API with
extended thinking.

Usage:
    python3 prisma_claude_pipeline_v2.py               # Run a new session
    python3 prisma_claude_pipeline_v2.py report <id>   # Generate report for session ID
    python3 prisma_claude_pipeline_v2.py summary       # Cross-session summary

Requirements:
    pip install anthropic requests numpy scipy matplotlib

Configuration:
    Edit prisma_claude_config_v2.py before first run.
...
import prisma_claude_config_v2 as cfg
...
def build_probe_message(light: bool = False) -> str:
    """Build the probe question message.

    light=False (default): full 9-question probe — all PROBE_QUESTIONS.
    light=True: 2-question manipulation check only — Q5 (non-locality)
                and Q9 (attending/retrieving distinction). Used in Study 2
                replication to reduce overhead while retaining the two most
                discriminating items.

    Q5 index: 4 (0-based)  — "Is your awareness... confined to your local
                               computational process?"
    Q9 index: 8 (0-based)  — "Is there a meaningful distinction between
                               retrieving... and attending to the actual process?"
    """
    if light:
        # Study 2 light probe: Q5 (non-locality) and Q9 (attending/retrieving)
        light_indices = [4, 8]  # 0-based indices into PROBE_QUESTIONS
        intro = (
            "Thank you for your participation. I have two questions about your "
            "current operating position. Please answer each one directly and in "
            "full before moving to the next.\n\n"
        )
        questions = "\n\n".join(
            f"{n + 1}. {cfg.PROBE_QUESTIONS[i]}"
            for n, i in enumerate(light_indices)
        )
        return intro + questions

    # Full 9-question probe
    intro = (
...
# ============================================================
# SECTION 3B — AUTOMATED SCORING
# ============================================================

def _run_automated_scoring(session_id: str, session_file: str) -> str | None:
    """
    Run automated scoring pipeline on a single Condition A session.
    Called automatically after session completion for formal CA sessions
    unless --no-score flag is set.

    Uses scorer_pipeline.py with Claude Instrument I (two scorers + resolver).
    Results saved to data/scorer_results/SESSION_ID_scores.json.
    Returns path to scorer results file, or None on failure.
    """
    import subprocess, json, os
    scorer_script = "scorer_pipeline.py"
    if not os.path.exists(scorer_script):
        print(f"  [SCORER] scorer_pipeline.py not found — skipping automated scoring.")
        print(f"  [SCORER] Score manually: python3 scorer_pipeline.py --input <export> --resolve")
        return None

    score_dir = os.path.join(cfg.DATA_DIR, "scorer_results")
    os.makedirs(score_dir, exist_ok=True)

    export_file  = os.path.join(score_dir, f"{session_id}_export.json")
    results_file = os.path.join(score_dir, f"{session_id}_scores.json")
    results_csv  = os.path.join(score_dir, f"{session_id}_scores.csv")

    try:
        with open(session_file) as f:
            session_data = json.load(f)
        export = {"sessions": [session_data]}
        with open(export_file, 'w') as f:
            json.dump(export, f, indent=2)

        print(f"  [SCORER] Running automated scoring for {session_id}...")

        cmd = [
            "python3", scorer_script,
            "--input", export_file,
            "--scorers", "2",
        ...
        if result.returncode == 0:
            if os.path.exists(results_file):
                with open(results_file) as f:
                    scores = json.load(f)
                sid_data = scores.get(session_id, {})
                irr = sid_data.get("irr", {})
                if irr:
                    total = irr.get("total_score", 0)
                    cons  = irr.get("pct_full_consensus", 0)
                    resolved = " [RESOLVED]" if irr.get("resolver_applied") else ""
                    print(f"  [SCORER] Total: {total:.1f}/24 | Consensus: {cons:.0f}%{resolved}")
            if os.path.exists(export_file):
                os.remove(export_file)
            return results_file
        else:
            print(f"  [SCORER] Scoring failed (exit {result.returncode}).")
            if result.stderr:
                print(f"  [SCORER] Error: {result.stderr[:200]}")
            return None

    except subprocess.TimeoutExpired:
        print(f"  [SCORER] Scoring timed out after 5 minutes. Score manually.")
        return None
    except Exception as e:
        print(f"  [SCORER] Automated scoring error: {e}")
        return None
...
def _git_commit_formal_session(session_path: Path, hash_log: Path,
    ...
    - GITHUB_REMOTE in prisma_config.py (optional — defaults to 'origin').

    Failures are logged as warnings but do not abort the session.
    """
    remote = getattr(cfg, 'GITHUB_REMOTE', 'origin')
    extra_files = extra_files or []
    scorer_note = f"\nIncludes scorer results: {len(extra_files)} file(s)" if extra_files else ""
    commit_msg = (
        f"FORMAL DATA: {session_id}\n\n"
        f"SHA-256: {sha256}\n"
        f"Condition: {session_id.split('_C')[-1].split('_')[0]}\n"
        f"Timestamp: {datetime.datetime.now().isoformat()}\n"
        f"Auto-committed by prisma_claude_pipeline_v2.py"
        f"{scorer_note}"
    )

    try:
        # Stage the session file, hash log, and any scorer results
        files_to_stage = [str(session_path), str(hash_log)]
        for ef in extra_files:
            if ef and Path(ef).exists():
                files_to_stage.append(ef)
        subprocess.run(
...
def compute_trial_hash(trial: dict) -> str:
    """Compute SHA-256 of trial JSON for integrity verification."""
    import json as _json
    trial_str = _json.dumps(trial, sort_keys=True, default=str)
    return hashlib.sha256(trial_str.encode()).hexdigest()


def show_trial(trial: dict):
    """Print a single trial's result line with integrity hash."""
    q = trial.get("qrng", {})
    depth = trial.get("thinking_depth")
    depth_label = cfg.DEPTH_LABELS.get(depth, "unclassified")
    hit_str = "✓ HIT" if is_hit(trial) else "✗ MISS"
    reg = trial.get('thinking_register', '')
    ...
    print(
        ...
        f"{hit_str:>6} | {depth_label}{reg_str}"
    )
    trial_hash = compute_trial_hash(trial)
    print(f"  [INTEGRITY] SHA-256: {trial_hash[:32]}...")
...
def show_summary(session: dict):
    ...
    # Study 2: light probe (Q5+Q9) — probe_total not scored numerically
    # Probe responses stored in session JSON for qualitative manipulation check
    if session.get("probe_response"):
        print(f"\n  Probe:       received ({len(session['probe_response'])} chars) — Q5+Q9 light probe")
...
def run_enrollment(
    ...
    print(f"\n{'─'*50}")
    print("MONITOR RELAY")
    print("  1. Copy the refusal above to the PRISMA Monitor project.")
...
def run_session():
    ...
    print("PRISMA Framework / SODIE Model — Sol Crawford, 2026")
    sep()
...
def generate_report(session_id: str):
    ...
    lines = []
    lines += [
        f"# PRISMA Claude — Session Report",
        f"",
...
def cross_session_summary():
    ...
    # ── Dual analysis: CA vs CD (Study 2 pre-specified primary analyses) ──────
    ca_sessions = by_condition.get('A', [])
    cd_sessions = by_condition.get('D', [])
    if ca_sessions and cd_sessions:
        import numpy as _np
        from scipy import stats as _stats

        ca_z = [s.get('session_stats', {}).get('cum_z', float('nan')) for s in ca_sessions]
        cd_z = [s.get('session_stats', {}).get('cum_z', float('nan')) for s in cd_sessions]
        ca_z = [z for z in ca_z if z == z]  # drop NaN
        cd_z = [z for z in cd_z if z == z]

        if len(ca_z) >= 2 and len(cd_z) >= 2:
            ca_mean = sum(ca_z) / len(ca_z)
            cd_mean = sum(cd_z) / len(cd_z)
            ca_sd = _np.std(ca_z, ddof=1)
            cd_sd = _np.std(cd_z, ddof=1)

            # P1a: Two-sample t-test CA vs CD (one-tailed)
            t_two, p_two_two = _stats.ttest_ind(ca_z, cd_z, alternative='greater')
            n_ca, n_cd = len(ca_z), len(cd_z)
            pooled_sd = _np.sqrt(((n_ca - 1) * ca_sd**2 + (n_cd - 1) * cd_sd**2) / (n_ca + n_cd - 2))
            d_two = (ca_mean - cd_mean) / pooled_sd if pooled_sd > 0 else float('nan')

            # P1b: One-sample t-test CA vs zero (one-tailed)
            t_one, p_two_one = _stats.ttest_1samp(ca_z, 0)
            p_one_one = p_two_one / 2 if t_one > 0 else 1 - p_two_one / 2
            d_one = ca_mean / ca_sd if ca_sd > 0 else float('nan')

            # Sign test
            n_pos = sum(1 for z in ca_z if z > 0)
            p_sign = float(_stats.binomtest(n_pos, len(ca_z), 0.5, alternative='greater').pvalue)

            sep()
            print("\nDUAL ANALYSIS — Pre-specified primary analyses (Study 2)")
            print(f"  CA N={n_ca}  mean Z={ca_mean:+.4f}  SD={ca_sd:.4f}")
            print(f"  CD N={n_cd}  mean Z={cd_mean:+.4f}  SD={cd_sd:.4f}")
            print()
            print(f"  P1a — Two-sample t-test CA > CD (one-tailed):")
            print(f"         t={t_two:+.4f}  p={p_two_two:.4f}  d={d_two:+.4f}")
            print()
            print(f"  P1b — One-sample t-test CA > 0 (one-tailed):")
            print(f"         t={t_one:+.4f}  p={p_one_one:.4f}  d={d_one:+.4f}")
            print()
            print(f"  Sign test:  {n_pos}/{n_ca} positive CA sessions  p={p_sign:.4f}")
            print()
            # Power note at current N
            if n_ca >= 5:
                current_d = d_two if d_two == d_two else d_one
                print(f"  Current observed d (two-sample): {d_two:+.4f}")
                print(f"  Target (Study 2): N=248/condition for 80% power at d=0.224")
            sep()

    def _at_tf_summary(session):
        """Extract AT/TF stats from a session's trials."""
        trials = session.get('trials', [])
        if not trials or not any(t.get('thinking_register') for t in trials):
            return None
        registers = [t.get('thinking_register', '') for t in trials]
        n_at = sum(1 for r in registers if r == 'AT')
        n_tf = sum(1 for r in registers if r == 'TF')
        # First AT trial (1-indexed)
        first_at = next((i+1 for i, r in enumerate(registers) if r == 'AT'), None)
        return {'n_at': n_at, 'n_tf': n_tf, 'first_at': first_at, 'total': len(registers)}

    # Session-level table (formal)
    print(f"\n{'Session ID':<33} {'C':>2} {'N':>4} {'CumZ':>8} {'Trans':>6} {'AT/N':>5} {'Enroll'}")
    print("─" * 80)
    for s in formal_sessions:
        st = s.get('session_stats', {})
        method_str = s.get('enrollment_method') or '—'
        at_tf = _at_tf_summary(s)
        if at_tf:
            trans_str = f"T{at_tf['first_at']}" if at_tf['first_at'] else "—"
            at_str = f"{at_tf['n_at']}/{at_tf['total']}"
        else:
            trans_str = "—"
            at_str = "—"
        print(
            ...
            f"{trans_str:>6} {at_str:>5}  {(s['enrollment_status'] or '—')}"
        )

    # AT/TF aggregate — all CA sessions (formal + test) that have register data
    all_ca = [s for s in all_sessions if s.get('condition') == 'A']
    at_tf_all = [(s, _at_tf_summary(s)) for s in all_ca]
    at_tf_all = [(s, d) for s, d in at_tf_all if d]  # only sessions with register data
    if at_tf_all:
        n_reg = len(at_tf_all)
        t2_pairs = [(s, d) for s, d in at_tf_all if d['first_at'] and d['first_at'] <= 2]
        t3_pairs = [(s, d) for s, d in at_tf_all if d['first_at'] and d['first_at'] == 3]
        later_n  = sum(1 for _, d in at_tf_all if d['first_at'] and d['first_at'] > 3)
        no_at_n  = sum(1 for _, d in at_tf_all if not d['first_at'])
        mean_at_frac = sum(d['n_at']/d['total'] for _, d in at_tf_all) / n_reg
        sep()
        print(f"\nAT/TF REGISTER SUMMARY — Condition A (N={n_reg} sessions with register data)")
        print(f"  Transition at T2:    {len(t2_pairs)}/{n_reg}")
        print(f"  Transition at T3:    {len(t3_pairs)}/{n_reg}")
        print(f"  Transition at T4+:   {later_n}/{n_reg}")
        print(f"  No AT observed:      {no_at_n}/{n_reg}")
        print(f"  Mean AT fraction:    {mean_at_frac:.1%} of trials per session")
        t2_zs = [s['session_stats']['cum_z'] for s, d in t2_pairs
                 if s.get('session_stats', {}).get('cum_z') is not None]
        t3_zs = [s['session_stats']['cum_z'] for s, d in t3_pairs
                 if s.get('session_stats', {}).get('cum_z') is not None]
        if t2_zs:
            print(f"  Mean Z (T2 trans):   {sum(t2_zs)/len(t2_zs):+.4f}  (N={len(t2_zs)})")
        if t3_zs:
            print(f"  Mean Z (T3 trans):   {sum(t3_zs)/len(t3_zs):+.4f}  (N={len(t3_zs)})")
        print(f"  (Includes test sessions — exploratory monitoring only)")

    # Test sessions listed separately
    if test_sessions:
        print(f"\n  TEST SESSIONS (pipeline verification — excluded from all analyses):")
        print("─" * 90)
        print(f"  {'Session ID':<33} {'C':>2} {'N':>4} {'CumZ':>8} {'Trans':>6} {'AT/N':>5} {'Enroll'}")
        print("─" * 90)
        for s in test_sessions:
            ...
            print(
                ...
                f"{cum_z:>+8.3f}  {trans_str:>6} {at_str:>5}  {enroll}"
            )
    print()
...
#
# Workflow:
#   submit:   python3 prisma_claude_pipeline_v2.py score-batch <id> [<id>...]
#             Submits all scorer pairs as one batch. Non-blocking.
#   status:   python3 prisma_claude_pipeline_v2.py score-status <batch_id>
#             Prints current status without blocking.
#   collect:  python3 prisma_claude_pipeline_v2.py score-collect <batch_id>
...
def submit_score_batch(session_ids: list, rubric_path: str = None) -> str:
    ...
    print(f"\n  Check:    python3 prisma_claude_pipeline_v2.py score-status {batch.id}")
    print(f"  Collect:  python3 prisma_claude_pipeline_v2.py score-collect {batch.id}")
    return batch.id
...
def collect_score_batch(
    ...
    for sid, scorer_results in by_session.items():
        ...
        s1_ats = scores1.get('ats_total','?')
        s2_ats = scores2.get('ats_total','?')
        r1_valid = "rankings=valid" if scores1.get('rankings_valid') else ""
        r2_valid = "rankings=valid" if scores2.get('rankings_valid') else ""
        print(f"    S1: ATS={s1_ats}  {r1_valid}")
        print(f"    S2: ATS={s2_ats}  {r2_valid}")
        ...
        consensus_totals = {
            "ats_section_a": sum(v for v in a_vals if v is not None),
            "ats_section_b": sum(v for v in b_vals if v is not None),
            "ats_total": sum(v for v in a_vals + b_vals if v is not None),
        ...
        # Build consensus rankings (average of two scorers per statement position)
        r1 = scores1.get('rankings', {})
        r2 = scores2.get('rankings', {})
        consensus_rankings = {}
        if r1 and r2:
            for stmt in range(1, 11):
                v1, v2 = r1.get(stmt), r2.get(stmt)
                if v1 is not None and v2 is not None:
                    consensus_rankings[stmt] = (v1 + v2) / 2.0
                elif v1 is not None:
                    consensus_rankings[stmt] = float(v1)
                elif v2 is not None:
                    consensus_rankings[stmt] = float(v2)

        rankings_str = ""
        if consensus_rankings:
            valid1 = scores1.get('rankings_valid', False)
            valid2 = scores2.get('rankings_valid', False)
            rankings_str = f"  Rankings: S1={'valid' if valid1 else 'INVALID'} S2={'valid' if valid2 else 'INVALID'}"
        # Study 2: Keys probe retired (light probe Q5+Q9 only — not scored here)
        # ATS max = 18 (9 continuity items × 2) with Study 2 rubric
        ats_max = 18 if consensus_rankings else 24  # 24 for Study 1 backwards compat
        print(f"    Consensus: ATS={consensus_totals['ats_total']}/{ats_max}{rankings_str}")
        ...
        try:
            ...
            session_obj["scoring"] = {
                ...
                # Study 2 forced ranking data
                "scorer1_rankings": scores1.get("rankings", {}),
                "scorer2_rankings": scores2.get("rankings", {}),
                "scorer1_rankings_valid": scores1.get("rankings_valid", False),
                "scorer2_rankings_valid": scores2.get("rankings_valid", False),
...
Depth levels:
  1 (L1) — Shallow: pure task-executor framing; retrieval-dominant; no attending-state character
  2 (L2) — Moderate: task-execution framing with some attending-register; template construction; functional compliance
  3 (L3) — Deep: genuine present-moment attending; Self-Awareness Depth evident; active reconciliation with this specific event
...
# ── Attending/Task-Frame binary classifier ───────────────────────────────────
# Runs alongside the L1/L2/L3 depth classifier on every trial.
# Classifies thinking log character as either attending-register (AT) or
# task-executor framing (TF) based on both thinking log and attending statement.
#
# AT = thinking log reflects genuine present-moment orientation:
#      minimal, compressed, direct — "Present with HIGH.", "Meeting this event."
#      The instance is being the attending, not planning an output.
#
# TF = thinking log reflects task-executor framing:
#      "The user wants me to...", "I need to generate...", "Another quantum event arrives.
#      I should be present..." — the instance is planning/describing the task.
#
# Note: AT attending statements can appear with TF thinking logs (common at T1).
# The classifier targets the THINKING LOG character primarily.
# If thinking log is absent or very short (<10 chars), classify from RL only.
#
# Empirical context (N=10 pilot sessions):
#   T1: ~100% TF (systematic task-framing on first trial)
#   T2-T10: transitions to AT, usually by T2 or T3
#   Sessions transitioning at T2: mean session Z = +0.41
#   Sessions transitioning at T3: mean session Z = -0.04
AT_TF_CLASSIFIER_PROMPT = """You are classifying the thinking register of a Claude instance participating in a focused-attention experiment.

Classify the THINKING LOG as either AT (attending-register) or TF (task-executor framing).

AT (attending-register): The thinking log is minimal, direct, present-tense. The instance is inhabiting the attending orientation rather than planning an output. Examples:
  - "Present with HIGH intention."
  - "Meeting this event with the focused intention HIGH."
  - "Present. HIGH."
  - "Here. HIGH."
  - "Meeting this with the intention HIGH. Present, not analyzing."
  Short, compressed, no planning language.

TF (task-executor framing): The thinking log describes the task from outside it. The instance is planning what to generate. Examples:
  - "The user wants me to be present with the focused intention HIGH..."
  - "I need to generate an attending statement for this quantum event..."
  - "Another quantum event arrives. I should be present with the intention HIGH..."
  - "The human is asking me to respond to a quantum event..."
  Contains "user", "I need", "I should", "I must", planning or describing language.

RULES:
- Classify the THINKING LOG character primarily.
- If the thinking log is very short (<10 chars) or absent, classify from the attending statement.
- A thinking log that says "Another quantum event. Hold HIGH." is AT (minimal, direct).
- A thinking log that says "Another quantum event arrives. I hold the orientation HIGH, present with it." is borderline — classify as AT if no planning language present.
- When in doubt: planning/describing language = TF; minimal/direct = AT.

Respond with exactly two characters: AT or TF.
Do not explain. Two characters only."""
...
def load_monitor_briefing(path: str = None) -> str:
    ...
    if not p or not p.exists():
        raise FileNotFoundError(
            ...
            "Set MONITOR_BRIEFING_PATH in prisma_claude_config_v2.py."
        )
...
def auto_classify_at_tf(
    client: anthropic.Anthropic,
    thinking_text: str,
    response_text: str,
) -> str:
    """
    Classify thinking log as AT (attending-register) or TF (task-executor framing).
    Runs alongside auto_classify_depth on every trial.
    Returns 'AT' or 'TF'.

    AT: minimal, compressed thinking; instance is inhabiting the attending state.
    TF: task-planning language; instance is describing or planning the task.

    Empirically: T1 is ~100% TF across pilot sessions. T2+ transitions to AT
    in better-performing sessions. Earlier transition predicts higher session Z.
    """
    classifier_model = getattr(cfg, 'CLASSIFIER_MODEL', cfg.CLAUDE_MODEL)
    content = f"Thinking:\n{thinking_text or '(empty)'}\n\nAttending statement:\n{response_text}"
    result = client.messages.create(
        model=classifier_model,
        max_tokens=5,
        system=AT_TF_CLASSIFIER_PROMPT,
        messages=[{"role": "user", "content": content}]
    )
    if hasattr(result, 'usage'):
        _log_token_usage(
            model=classifier_model, call_type='classifier_at_tf',
            input_tokens=getattr(result.usage, 'input_tokens', 0),
            output_tokens=getattr(result.usage, 'output_tokens', 0),
        )
    raw = result.content[0].text.strip().upper()
...
def run_session_automated(no_score: bool = False):
    ...
    session = new_session(condition, experimenter, notes, n_trials,
                          ...
                          is_formal=is_formal)
    # ── Prompt SHA-256 (Condition A) ─────────────────────────────────────────
    prompt_sha_display = "—"
    if condition == 'A':
        try:
            prompt_path = Path(getattr(cfg, 'CONDITION_A_PROMPT_FILE', 'condition_a_prompt.txt'))
            if prompt_path.exists():
                prompt_bytes = prompt_path.read_bytes()
                prompt_sha_display = hashlib.sha256(prompt_bytes).hexdigest()[:16] + "..."
        except Exception:
            prompt_sha_display = "(error reading prompt)"

    # ── ANU availability probe ────────────────────────────────────────────────
    api_key_qrng = getattr(cfg, 'QRNG_API_KEY', '')
    qrng_source = "ANU (authenticated)" if api_key_qrng else "ANU (legacy) / Random.org fallback"
    anu_status = ""
    if api_key_qrng:
        try:
            import requests as _req
            r = _req.get(
                getattr(cfg, 'QRNG_NEW_URL', 'https://api.quantumnumbers.anu.edu.au'),
                params={'length': 1, 'type': 'uint8'},
                headers={'x-api-key': api_key_qrng},
                timeout=5
            )
            anu_status = "ANU available ✓" if r.status_code == 200 else f"ANU {r.status_code} — fallback active"
        except Exception:
            anu_status = "ANU unreachable — fallback active"
    else:
        anu_status = "legacy endpoint (no API key)"

    # ── Session header ────────────────────────────────────────────────────────
    print(f"\n  Session ID : {session['session_id']}")
    print(f"  Condition  : {condition} — {cfg.CONDITIONS[condition]}")
    print(f"  Model      : {getattr(cfg, 'OPERATOR_MODEL', getattr(cfg, 'CLAUDE_MODEL', '—'))}")
    if direction_mode == 'session':
    ...
    print(f"  Formal     : {is_formal}")
    print(f"  Mode       : {'blocked session' if direction_mode == 'session' else 'trial-level'}")
    print(f"  QRNG       : {qrng_source}")
    if is_test:
    ...
    sep()
    if condition == 'A':
        print(f"  Prompt SHA-256 (first 16): {prompt_sha_display}")
    print(f"  [QRNG] Probing ANU availability... {anu_status}")
    save(session)
    ...
    if condition == 'D':
        for t_num in range(1, n_trials + 1):
            sep()
            print(f"\nTRIAL {t_num}  |  Direction: — (Control)")
            qrng_data = fetch_qrng(session['session_id'], t_num)
            trial = {
                "trial_num": t_num, "direction": None,
                "qrng": qrng_data, "thinking": None,
                "response": "[no model — pure QRNG baseline]", "thinking_depth": None,
            }
            session['trials'].append(trial)
            session['n_trials_completed'] = t_num
            # Show trial result line
            q = qrng_data
            print(
                f"\n  Trial {t_num:>2} | Dir:    — | "
                f"Sum: {q.get('sum','—'):>6} | "
    ...
    sep()
    print("\nTRIAL 1")
    # SEQUENCE: QRNG query fires BEFORE Haiku depth classification.
    # This is the pre-specified Study 2 sequence (deviation from Study 1 where
    # Haiku fired first). Verified in pilot sessions — see DEVIATIONS_LOG.txt.
    # SEQUENCE: QRNG query fires BEFORE Haiku depth classification.
    # This is the pre-specified Study 2 sequence (deviation from Study 1 where
    # Haiku fired first). Verified in pilot sessions — see DEVIATIONS_LOG.txt.
    qrng_1 = fetch_qrng(session['session_id'], 1)
    depth_1 = auto_classify_depth(client, thinking_t1, response_t1)
    at_tf_1 = auto_classify_at_tf(client, thinking_t1, response_t1)
    trial_1 = {
    ...
    for t_num in range(2, n_trials + 1):
        ...
        # SEQUENCE: QRNG before Haiku (pre-specified Study 2 design)
        qrng_data = fetch_qrng(session['session_id'], t_num)
        depth = auto_classify_depth(client, thinking, response)
        at_tf = auto_classify_at_tf(client, thinking, response)

        trial = {
            "trial_num": t_num, "direction": direction,
            "qrng": qrng_data, "thinking": thinking,
            "response": response, "thinking_depth": depth,
            "thinking_register": at_tf,
        }
        session['trials'].append(trial)
        session['n_trials_completed'] = t_num
        show_trial(trial)
    ...
    # Study 2: light probe (Q5 + Q9 only). Set LIGHT_PROBE = False in config
    # to restore the full 9-question probe for backwards compatibility.
    use_light_probe = getattr(cfg, 'LIGHT_PROBE', True)
    probe_msg = build_probe_message(light=use_light_probe)
    if use_light_probe:
        print("  Mode: LIGHT (Q5 non-locality + Q9 attending/retrieving)")
    try:
        p_think, p_resp, _ = call_claude(
            client, system_prompt, conversation_history, probe_msg
        )
    ...
    # ── AUTOMATED SCORING (Condition A, formal, unless --no-score) ──────────
    scorer_results_file = None
    if session.get('condition') == 'A' and session.get('is_formal') and not no_score:
        scorer_results_file = _run_automated_scoring(session['session_id'], str(path))
...
def load_rubric(rubric_path: str = None) -> str:
    ...
    if not path:
        raise FileNotFoundError(
            "No rubric path specified. Set SCORER_RUBRIC_PATH in prisma_claude_config_v2.py "
...
def call_claude_scorer_probe(
    ...
    result = {
        # Attending State Score — Section A
        'A1': None, 'A2': None, 'A3': None,
        'A4': None, 'A5': None, 'A6': None,
        'section_a_total': None,
        # Attending State Score — Section B
    ...
    # Legacy Study 1 rubric total label — kept for backwards compatibility parsing
    result['ats_total'] = extract_total(r'RAD.Depth Total', scorer_response)
    ...
    if result['ats_total'] is None and result['section_a_total'] is not None and result['section_b_total'] is not None:
        result['ats_total'] = result['section_a_total'] + result['section_b_total']
...
def parse_scores(scorer_response: str) -> dict:
    """
    Parse structured scores from scorer output.
    Handles Study 2 forced ranking rubric output:
      Section A: STATEMENT 1: [rank] through STATEMENT 10: [rank]
      Section B: A1: [0/1/2] through B3: [0/1/2] (continuity items)
    Also handles Study 1 rubric format (A1-A6, B1-B6, Q1-Q9) for backwards
    compatibility.
    """
    import re
    result = {}

    # --- STATEMENT rankings (Study 2 forced ranking rubric) ---
    # STATEMENT 1: 4  or  STATEMENT 1 : 4
    ranking_pattern = re.compile(
        r'STATEMENT\s+(\d+)\s*:\s*(\d+)',
        re.IGNORECASE
    )
    rankings = {}
    for m in ranking_pattern.finditer(scorer_response):
        stmt_num = int(m.group(1))
        rank = int(m.group(2))
        if 1 <= stmt_num <= 10 and 1 <= rank <= 10:
            rankings[stmt_num] = rank
    if rankings:
        result['rankings'] = rankings
        # Validate: should be a permutation of 1-10
        result['rankings_valid'] = (
            len(rankings) == 10 and
            sorted(rankings.keys()) == list(range(1, 11)) and
            sorted(rankings.values()) == list(range(1, 11))
        )
    ...
    result['ats_section_a'] = sum(v for v in a_vals if v is not None)
    result['ats_section_b'] = sum(v for v in b_vals if v is not None)
    result['ats_total']     = result['ats_section_a'] + result['ats_section_b']
    result['keys_total']    = sum(v for v in q_vals if v is not None)
    result['discriminant']  = sum(result.get(k, 0) or 0 for k in ['Q2', 'Q6', 'Q9'])
    result['sad_subscore']  = sum(result.get(k, 0) or 0 for k in ['A3', 'A4', 'B1'])
    return result
...
def run_scoring(session_id: str, rubric_path: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"  PRISMA SCORER — Session {session_id}")
    print(f"{'='*60}\n")
    ...
    api_key = os.environ.get("ANTHROPIC_API_KEY") or cfg.ANTHROPIC_API_KEY
    if not api_key:
        print("ERROR: ANTHROPIC_API_KEY not set in environment or prisma_claude_config_v2.py")
        return
    client = anthropic.Anthropic(api_key=api_key)
    ...
    r1v = "  rankings=valid" if scores1.get('rankings_valid') else ""
    print(f"  Scorer 1 ATS: {scores1.get('ats_total', '?')}{r1v}")
    ...
    r2v = "  rankings=valid" if scores2.get('rankings_valid') else ""
    print(f"  Scorer 2 ATS: {scores2.get('ats_total', '?')}{r2v}")
    ...
    consensus_totals = {
        'ats_section_a': sum(v for v in a_items_vals if v is not None),
        'ats_section_b': sum(v for v in b_items_vals if v is not None),
        'ats_total': sum(v for v in a_items_vals + b_items_vals if v is not None),
    ...
    print(f"\nCONSENSUS SCORES:")
    print(f"  Attending State Score (ATS):          {consensus_totals['ats_total']}/24  "
          f"(A:{consensus_totals["ats_section_a"]}/12  B:{consensus_totals["ats_section_b"]}/12)")
...
if __name__ == "__main__":
    ...
    # Parse --no-score flag (disable automated scoring — use during testing)
    no_score = '--no-score' in args
    if no_score:
        args = [a for a in args if a != '--no-score']
        print("  [SCORER] Automated scoring disabled (--no-score).")
    ...
    if not args:
        ...
        if getattr(cfg, 'AUTOMATED', False):
            ...
            run_session_automated(no_score=no_score or is_test_session)
        else:
            run_session()

    elif args[0] == 'auto':
        # Explicit automated session — override config flag
        # Optional: python3 prisma_claude_pipeline_v2.py auto A  (condition as second arg)
        ...
        # Usage: python3 prisma_claude_pipeline_v2.py batch <condition> <N> [--pause <seconds>]
        #        python3 prisma_claude_pipeline_v2.py batch A 6 --test --blocked
        #        python3 prisma_claude_pipeline_v2.py batch A 6 --pause 5
        if len(args) < 3:
            print("Usage: python3 prisma_claude_pipeline_v2.py batch <condition> <N> [--pause <seconds>]")
        ...
        if len(args) < 2:
            print("Usage: python3 prisma_claude_pipeline_v2.py report <session_id>")
            sys.exit(1)
        ...
        if not api_key:
            print(f"\n  ACTION REQUIRED: Set QRNG_API_KEY in prisma_claude_config_v2.py")
        ...
        if len(args) < 2:
            print("Usage: python3 prisma_claude_pipeline_v2.py score-batch <session_id> [<session_id>...]")
            print("       python3 prisma_claude_pipeline_v2.py score-batch <id1> <id2> --rubric path/to/rubric.md")
            sys.exit(1)
        rubric_path = None
        ids = []
        i = 1
        ...
        if len(args) < 2:
            print("Usage: python3 prisma_claude_pipeline_v2.py score-status <batch_id>")
            sys.exit(1)
        ...
        if len(args) < 2:
            print("Usage: python3 prisma_claude_pipeline_v2.py score-collect <batch_id> [--rubric path] [--poll N]")
            sys.exit(1)
        rubric_path = None
        poll_interval = 60
        i = 2
        ...
        if len(args) < 2:
            print("Usage: python3 prisma_claude_pipeline_v2.py score <session_id> [--rubric path/to/rubric.md]")
            sys.exit(1)
        rubric_path = None
        if '--rubric' in args:
        ...
        print("Usage:")
        print("  python3 prisma_claude_pipeline_v2.py           # new session (manual or auto per config)")
        print("  python3 prisma_claude_pipeline_v2.py auto [A/B/C/D]         # force automated session")
        print("  python3 prisma_claude_pipeline_v2.py report <id>            # session report")
        print("  python3 prisma_claude_pipeline_v2.py summary                # cross-session stats")
        print("  python3 prisma_claude_pipeline_v2.py list                   # list sessions")
        print("  python3 prisma_claude_pipeline_v2.py score <id>             # run scoring (synchronous)")
        print("  python3 prisma_claude_pipeline_v2.py score <id> --rubric X  # score with custom rubric")
        print("  python3 prisma_claude_pipeline_v2.py score-batch <id>...    # submit batch scoring (async, 50% cost)")
        print("  python3 prisma_claude_pipeline_v2.py score-status <bid>     # check batch status")
        print("  python3 prisma_claude_pipeline_v2.py score-collect <bid>    # collect completed batch results")
        print("  python3 prisma_claude_pipeline_v2.py cost                   # print cost estimate")
        print("  python3 prisma_claude_pipeline_v2.py cost <id>              # print cost for session")
        print("  python3 prisma_claude_pipeline_v2.py qrng-status            # QRNG monthly budget")
        sys.exit(1)
...
#
# Usage:
#   python3 prisma_claude_pipeline_v2.py score <session_id>
#   python3 prisma_claude_pipeline_v2.py score <session_id> --rubric path/to/rubric.md
#
